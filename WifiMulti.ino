...
bool WifiMul::addAP(const char* ssid, const char *passphrase)
{
    WifiAPlist_t newAP;
...
uint8_t WifiMul::run(uint32_t connectTimeout)
{
    int8_t scanResult;
    ...
    UI.tft.drawArc(120, 120, 115, 110, 300, 360, TFT_BLACK, TFT_BLACK);
    ...
    if(scanResult == WIFI_SCAN_RUNNING) {
        ...
        if(scanResult == 0) {
            ...
            for(int8_t i = 0; i < scanResult; ++i) {
                UI.tft.drawArc(120, 120, 115, 110, 300-((i+1)/scanResult*120), 360, TFT_BLACK, TFT_BLACK);
                String ssid_scan;
                int32_t rssi_scan;
                uint8_t sec_scan;
                uint8_t* BSSID_scan;
        ...
        if(bestNetwork.ssid) {
            ...
            while(status != WL_CONNECTED && status != WL_NO_SSID_AVAIL && status != WL_CONNECT_FAILED && (millis() - startTime) <= connectTimeout) {
                delay(10);
                UI.tft.drawArc(120, 120, 115, 110, 180-((millis()-startTime)*180/connectTimeout), 360, TFT_BLACK, TFT_BLACK);
                status = WiFi.status();
            }
